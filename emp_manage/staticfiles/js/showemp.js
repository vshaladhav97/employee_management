    ...
    $.each(
        ...
        function(idx, course) {
            $("#courserows").append(
                ...
                "<tr><td>{% if request.user.is_superuser %}<button  onclick='changes(" + course.id + ")''>Update</button><button id = 'refresh' onclick='deleteCourse(" + course.addressdetails + ")'>Delete</button>{% else %} {% endif %}{% endblock %}</td><td class='editable' data-id=" + course.id + " data-type='first_name'>" +
