...
def sign_up(request):
    """This sign_up() function is used to perform sign up of user and assign group to the user."""
...
def user_login(request):
    """This user_login() function is used to perform login of existing user."""
    if request.method == 'POST':
...
class Management(APIView):
    """This class is used to perform get and post data in database through rest framework"""
...
def save_data_test(request, id):
    """This save_update() is used to perform updating row in database through rest framework."""
    if request.method == "POST":
